#! /usr/bin/python3.7
# encoding=utf-8
"""Interictal tab layout -- standard Qt theme (no custom stylesheets/backgrounds),
same redesign rules as elec_form.py/ictal_form.py. No patient-name or subject-dir
fields -- the subject is whatever's selected in the Patients panel, resolved
automatically server-side. The whole content area is disabled with a status_label
explaining why until a subject is selected.
"""
...
class Interictal_gui(object):

    def setupUi(self, InterModule):

        # main window
        InterModule.setObjectName("InterModule")
        self.setWindowTitle('InterIctal Computation')
        self.setAttribute(Qt.WA_MacShowFocusRect, 0)
        outer_layout = QGridLayout()

        # status banner -- same pattern as elec_form.py/ictal_form.py
        ...
        # whole based on subject selection (see client_inter.py's set_subject)
        ...
        self.leftPannelLayout.addWidget(self.canvas, 1, 1, 25, 26)
        ...
        self.gridlayout.setColumnStretch(1, 26)
        ...
        self.rightPannelLayout.addWidget(self.compu_frame, 20, 1, 5, 2)
        ...
        self.disp_filter_low.setText('80')
        ...
        self.disp_filter_high.setText('250')
        ...
        #### relative thresh
        self.relTh_label = QLabel(self.content)
        self.relTh_label.setText('relThr(mul)')
        self.compu_frame_layout.addWidget(self.relTh_label, 1, 1, 1, 1)
        self.lineedit_relTh_name = QLineEdit(self.content)
        self.lineedit_relTh_name.setText('2.0')
        self.lineedit_relTh_name.setToolTip('threshold based on per-channel envelope median')
        self.compu_frame_layout.addWidget(self.lineedit_relTh_name, 1, 2, 1, 1)
        #### abs thresh
        self.absTh_label = QLabel(self.content)
        self.absTh_label.setText('absThr(mul)')
        self.compu_frame_layout.addWidget(self.absTh_label, 1, 3, 1, 1)
        self.lineedit_absTh_name = QLineEdit(self.content)
        self.lineedit_absTh_name.setText('2.0')
        self.lineedit_absTh_name.setToolTip('threshold based on whole-data envelope median')
        self.compu_frame_layout.addWidget(self.lineedit_absTh_name, 1, 4, 1, 1)
        #### min gap
        self.minGap_label = QLabel(self.content)
        self.minGap_label.setText('minGap(ms)')
        self.compu_frame_layout.addWidget(self.minGap_label, 2, 1, 1, 1)
        self.lineedit_minGap_name = QLineEdit(self.content)
        self.lineedit_minGap_name.setText('20')
        self.lineedit_minGap_name.setToolTip('two events away less than minGap will be merged')
        self.compu_frame_layout.addWidget(self.lineedit_minGap_name, 2, 2, 1, 1)
        #### min last
        self.minDur_label = QLabel(self.content)
        self.minDur_label.setText('minDur(ms)')
        self.compu_frame_layout.addWidget(self.minDur_label, 2, 3, 1, 1)
        self.lineedit_minDur_name = QLineEdit(self.content)
        self.lineedit_minDur_name.setText('50')
        self.lineedit_minDur_name.setToolTip('events less than minDur will be dropped')
        self.compu_frame_layout.addWidget(self.lineedit_minDur_name, 2, 4, 1, 1)
        # HI dets
        self.HI_button = QPushButton(self.content)
        self.HI_button.setText('HFO detection')
        self.HI_button.setToolTip('detect high frequency events(HFO)')
        self.compu_frame_layout.addWidget(self.HI_button, 3, 1, 1, 4)
        self.HI_button.clicked.connect(self.HI_computation_func)
        self.HI_button.setEnabled(False)
        # show counts
        self.HI_proBar = QProgressBar(self.content)
        self.HI_proBar.setValue(0)
        self.compu_frame_layout.addWidget(self.HI_proBar, 4, 1, 1, 4)
        #show dets filt
        self.hiDetsFilt_button = QPushButton(self.content)
        self.hiDetsFilt_button.setText('HFO dets (filt)')
        self.hiDetsFilt_button.setToolTip('show HFO detections with filtered data as background')
        self.compu_frame_layout.addWidget(self.hiDetsFilt_button, 5, 1, 1, 2)
        self.hiDetsFilt_button.setEnabled(False)
        #show dets raw
        self.hiDetsRaw_button = QPushButton(self.content)
        self.hiDetsRaw_button.setText('HFO dets (raw)')
        self.hiDetsRaw_button.setToolTip('show HFO detections with raw data as background')
        self.compu_frame_layout.addWidget(self.hiDetsRaw_button, 5, 3, 1, 2)
        self.hiDetsRaw_button.setEnabled(False)
        # show main window
        ...
        # the whole content area starts disabled -- client_inter.py's set_subject
