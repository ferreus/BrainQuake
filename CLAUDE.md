...
- **Phase (c) — Docker image**: done — split into a hierarchical two-image build to keep the public image small and avoid rebuilding FreeSurfer/FSL on every app change: `v2/docker/base.Dockerfile` (→ `brainquake-base`, built separately via `v2/docker/build-base.sh`, not by compose) holds `FROM ubuntu:22.04` + FreeSurfer **7.4.1** (matching the legacy app's version, not latest 8.2.0 — upgrade deferred until v2 is otherwise validated, official Ubuntu22 tarball) + FSL's `fsl-flirt` conda package only (installed via micromamba from FSL's own conda channel — `flirt` is the only FSL binary the codebase calls; this replaced a full `fslinstaller.py` install that cost ~10.5GB for that one binary) + `hough-3d-lines` built from source in a discarded builder stage. `v2/docker/Dockerfile` (→ `brainquake-server`, `api`+`worker` in `docker-compose.yml`) is `FROM brainquake-base` and adds only the Python venv + app code — this is the file to edit for a new apt/pip dependency, since it never touches the base layers. `docker-compose.yml` also has a `web` service (`v2/web/Dockerfile`, multi-stage node build → nginx) so `docker compose up --build` serves the browsable UI alongside `api`/`worker`. FS_LICENSE mounted at runtime, never baked in. Validated: both containers build/boot healthy, all native binaries (`recon-all`, `flirt`, `hough3dlines`) run, DB round-trip works. A real end-to-end `recon-all` run (needs the user's own `FS_LICENSE` + hours) is still open. DEV note: `base.Dockerfile` currently reads the FreeSurfer tarball from a local cache dir via a buildx additional-context (`build-base.sh`) instead of `wget`, to speed up iteration — see the commented-out block in the Dockerfile to restore the portable path.
